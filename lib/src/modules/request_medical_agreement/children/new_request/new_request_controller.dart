...
part 'new_request_model.dart';

class NewRequestController extends GetxController
    with StateMixin<_NewRequestModel> {
  final ThreadsService threadsService = Get.find();
  final RequestMedicalAgreementRepository apiConn = Get.find();
  ...
  List<BreadcrumbWeb> breadcrumbs = [
    ...
    BreadcrumbWeb(
      'Solicitar Convenio Médico',
      route: RequestMedicalAgreementPage.page.name
    ),
    BreadcrumbWeb(msg.newRequest.tr()),
  ];
  ...
  Future<void> loadAllCatalogs() async {
    ...
    change(
      _NewRequestModel.empty().copyWith(
        ...
        func: () async {
          ...
          appService.alert.show(
            ...
            message: wasSuccessful
                ? msg.requestRegistered.tr()
                : msg.requestNotRegistered.tr(),

...
class FilterPageController extends GetxController
  ...
  /// Arguments for the page, used when not navigating with Get.toNamed
  ...
  Future<void> onInit() async {
    ...
    if (Get.arguments != null) {
      ...
      final tmp = args['item'] as ItemDirectoryMdl;
      isWeb = args['isWeb'] as bool? ?? false;

      await setTypeSearch(tmp);
    }
  ...
  Future<void> setTypeSearch(ItemDirectoryMdl itemSelected) async {
    ...
    Map<String, dynamic> sourceArgs = {};

    /// Determine the source of the arguments
    /// (mobile navigation or web direct render)
    if (arguments != null) {
      sourceArgs = arguments!;
    } else if (Get.arguments != null) {
      sourceArgs = Get.arguments;
    }

    /// We receive the catalogs
    catEspecialidades =
        sourceArgs['catEspecialidades'] as List<Map<String, dynamic>>? ?? [];
    catCirculos =
        sourceArgs['catCirculos'] as List<Map<String, dynamic>>? ?? [];
    catPlanHospitalario =
        sourceArgs['catPlanHospitalario'] as List<Map<String, dynamic>>? ?? [];
    catClinicas =
        sourceArgs['catClinicas'] as List<Map<String, dynamic>>? ?? [];
    catOtrosServicios =
        sourceArgs['catOtrosServicios'] as List<Map<String, dynamic>>? ?? [];
    catEstados = sourceArgs['catEstados'] as List<Map<String, dynamic>>? ?? [];
  ...
  Future<bool> fetchDataCirculos() => appService.threads.execute(
    customExceptionMessages: {
      Exception(): ExceptionAlertProperties(
        message:
            '''${msg.errorGetting.tr()} ${msg.circle.pValue}. ${msg.pleaseTryAgainLater.tr()}''',
      ),
    },
    func: () async {
      if (catCirculos.isEmpty) {
        ...
        catCirculos = [
          {'claveCirculoMedico': '0', 'circuloMedico': msg.all.tr()},
          ...tmpData.map(
            (e) {
  ...
  Future<bool> fetchDataEspecialidades() => appService.threads.execute(
    customExceptionMessages: {
      Exception(): ExceptionAlertProperties(
        message:
            '''${msg.errorGetting.tr()} ${msg.specialty.pValue}. ${msg.pleaseTryAgainLater.tr()}''',
      ),
    },
    func: () async {
      if (catEspecialidades.isEmpty) {
        ...
        catEspecialidades = [
          {'claveEspecialidad': '0', 'especialidad': msg.all.tr()},
          ...tmpData.map(
            (e) => {
      ...
      titleSearchby = msg.doctor.tr();

      /// Hint text for the search input List
      hintTextSearch = msg.searchForSpecialty.tr();

      /// Label text for the search input
      labelSearchby = msg.optionalName.tr();
  ...
  Future<bool> fetchDataPlanHospitalario() => appService.threads.execute(
    customExceptionMessages: {
      Exception(): ExceptionAlertProperties(
        message:
            '''${msg.errorGetting.tr()} ${msg.healthPlan.pValue}. ${msg.pleaseTryAgainLater.tr()}''',
      ),
    },
    func: () async {
      ...
      /// Hint text for the search input List
      hintTextSearch = msg.lookingforHospital.tr();

      /// Label text for the search input
      titleSearchby = msg.hospital.tr();
  ...
  Future<bool> fetchDataClinicas() => appService.threads.execute(
    customExceptionMessages: {
      Exception(): ExceptionAlertProperties(
        message:
            '${msg.errorGetting.tr()} ${msg.clinicType.pValue}. '
            '${msg.pleaseTryAgainLater.tr()}',
      ),
    },
    func: () async {
      if (catClinicas.isEmpty) {
        ...
        catClinicas = [
          {'claveTipoClinica': '0', 'tipoClinica': msg.all.tr()},
          ...tmpData.map(
            (e) => {
      ...
      /// Hint text for the search input List
      hintTextSearch = msg.lookingforClinicaName.tr();

      /// Label text for the search input
      titleSearchby = msg.clinica.tr();
  ...
  Future<bool> fetchDataOtrosServicios() => appService.threads.execute(
    customExceptionMessages: {
      Exception(): ExceptionAlertProperties(
        message:
            '''${msg.errorGetting.tr()} ${msg.servicesType.pValue}. ${msg.pleaseTryAgainLater.tr()}''',
      ),
    },
    func: () async {
      if (catOtrosServicios.isEmpty) {
        ...
        catOtrosServicios = [
          {'claveTipoProveedor': '0', 'tipoProveedor': msg.all.tr()},
          ...tmpData.map(
            (e) => {
      ...
      /// Hint text for the search input
      hintTextSearch = msg.lookingforOtherServicesName.tr();

      titleSearchby = msg.otherServices.tr();
      labelSearchby = hintTextSearch;
  ...
  Future<bool> fetchDataMunicipios() {
    ...
    return appService.threads.execute(
      customExceptionMessages: {
        Exception(): ExceptionAlertProperties(
          message: msg.errorGettingMunicipalities.tr(),
        ),
      },
      func: () async {
        /// We obtain the data for the municipalities
        ...
        catMunicipios = [
          {'claveMunicipio': '0', 'municipio': msg.all.tr()},
          ...tmpData.map(
            (e) => {
  ...
  Future<void> onTapSearch(Map<String, dynamic> item) async {
    ...
    searchCtrler.text = item[keyToSearch.value] ?? '';

    /// If the filter is for doctors
    if (state!.itemSelected == 'medicos') {
      especialidadSelected.value = item;
    }

    /// If the filter is for hospitals
    if (state!.itemSelected == 'hospitales') {
      planHospitalarioSelected.value = item;
    }

    /// If the filter is for clinics
    if (state!.itemSelected == 'clinicas') {
      clinicasSelected.value = item;
    }

    /// If the filter is for other services
    if (state!.itemSelected == 'otros_servicios') {
      otrosServiciosSelected.value = item;
    }

    /// Navigate to the results page
    await goToFilterResultsPage();
  }

  /// Go back to the previous page, sending the updated catalogs
  void goBack() {
    Get.back(
      result: {
  ...
  Future<void> goToFilterResultsPage({bool isSearchByButton = false}) async {
    ...
    await Get.toNamed(
      ...
      arguments: ItemToSearchDirectoryMdl(
        ...
        isSearchByButton: isSearchByButton
      ),
    )!.then((_) {
  ...
  /// Navigates to the Filter Results Page with the selected filters.
  Future<void> goToFilterResultsWebPage({bool isSearchByButton = true}) async {
    final searchData = ItemToSearchDirectoryMdl(
      idPage: itemSelected.idPage,
      itemSelected: itemSelected,
      especialidad: especialidadSelected,
      circuloMedico: circuloSelected,
    ...
    await directoryController.showResultsWebPage(searchData);
  }
